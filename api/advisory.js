...
export default async function handler(req, res) {
  ...
  const relevant = searchRegulations(question, regulationIds);
...
{"answer":"<answer>","citations":[{"regulation":"","issuer":"","version":"","clause":"","excerpt":""}],"confidence":<0-100>,"riskFlag":"critical|high|medium|low|none","relatedClauses":[],"disclaimer":"Advisory only. Consult legal counsel."}`;
  ...
  try {
    ...
    try { const m=raw.match(/\{[\s\S]*\}/); parsed=JSON.parse(m?m[0]:raw); }
      ...
      parsed={answer:raw,citations:relevant.slice(0,2).map(c=>({regulation:c.title,issuer:c.issuer,version:c.version,clause:'See regulation',excerpt:c.text.substring(0,200)})),
        confidence:60,riskFlag:'medium',relatedClauses:[],disclaimer:'Advisory only.'};
    }
...
function searchRegulations(question, filterIds) {
  const q = question.toLowerCase();
  const keywords = q.split(/\s+/).filter(w=>w.length>3);
  const results=[];
  const regs = filterIds?.length
    ? Array.from(regulations.values()).filter(r=>filterIds.includes(r.id))
    : Array.from(regulations.values());
